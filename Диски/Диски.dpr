...
// Requaring and checking variables (D[i], CD[i], N)
  ...
  // different types
  If ((CFinish < CDN[1]) And (CFinish < CDN[2]) and (CFinish < CDN[3])) and (CFinish <> 0) then
  Begin
    case C of
      1: begin
      ...
      4: begin
           ...
           Writeln(CFinish:7:2, ' рублей');
         end;
    end;

    Write('Выгода, покупая данным способом, а не ');
    Writeln('только дискетами: ', (CDN[1]-CFinish):6:2);
    Write('Выгода, покупая данным способом, а не ');
    Writeln('только коробками: ', (CDN[2]-CFinish):6:2);
    Write('Выгода, покупая данным способом, а не ');
    Writeln('только ящиками: ', (CDN[3]-CFinish):6:2);
  End;
